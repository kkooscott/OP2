...
//            bool = false;
//        }
        /*if( $('#OP_CASE_DATE_S').val() == "" ){
            msg += '公告期滿日期起日，必須輸入<br/>';
            bool = false;
        }
        if( $('#OP_CASE_DATE_E').val() == "" ){
            msg += '公告期滿日期迄日，必須輸入<br/>';
            bool = false;
        }*/
        //受理日期起迄
        ...
        //受理拾得日期及公告期滿日期至少輸入一個條件，且日期區間不得超過一年
            if($('#OP_CASE_DATE_S').val() == "" && $('#OP_CASE_DATE_E').val() == "" && $('#OP_SEARCH_AC_DATE_S').val() == "" && $('#OP_SEARCH_AC_DATE_E').val() == "" ){
                msg += '受理日期及公告期滿日期至少輸入一個條件<br/>';
                bool = false;
            }else{
                if( $('#OP_SEARCH_AC_DATE_S').val() == "" && $('#OP_SEARCH_AC_DATE_E').val() == "")
                {
        ...
        if (bool){
            downloadReport("ReportServlet", {
                ajaxAction: 'OP07A12Q.xls',
                reportName: 'oP07A12Q.xlsx',
                newReportName: '期滿遺失物拾得人清冊.xlsx',
                ...
                printPUPO_NM_YN:$('input[name=printPUPO_NM_YN]:checked').val(),
                printPUPO_ADDR_YN:$('input[name=printPUPO_ADDR_YN]:checked').val(),
                printPUPO_TEL_YN:$('input[name=printPUPO_TEL_YN]:checked').val(),
                lockType : 'A'
            });
        }
    });
