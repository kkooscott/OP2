...
function registButton() {
    ...
    $('#tabInfomation5').click(function () {
        ...
        if (checkYNDetail == 'Y') {
            ...
            $('#tab_4_7_main').show();
            $('#tab_4_7_alert').hide();
            getIAnDlGridList($('#IPUBASIC_OP_SEQ_NO').val(), "OP04A01Q"); //取得畫面資料
        } else {
            $('#saveIAnDlBtn').hide();
            $('#tab_4_7_main').hide();
            $('#tab_4_7_alert').show();
    ...
    if (bool) {
        ...
        $(gridId).jqGrid({
            ...
            postData: {
                ...
                OP_NTC_GB_REC: $("#OP_SEARCH_NTC_GB_REC").val(),
                OP_NTC_GB_REC_NM: $('#OP_SEARCH_NTC_GB_REC').val() == '' ? '' : $('#OP_SEARCH_NTC_GB_REC option:selected').text(),
                OP_PUPO_DOC_WD: $("#OP_SEARCH_PUPO_DOC_WD").val(),
                OP_PUPO_DOC_NO: $("#OP_SEARCH_PUPO_DOC_NO").val(),
                OP_PU_YN_OV500: '1',
                OP_DEL_FLAG: '0',
                ACTION_TYPE: 'OP04A01Q_01',
