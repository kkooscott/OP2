...
function registButton() {
    ...
    //受理資料與明細按鈕事件
    BasicAndDetailButton("OP08A03Q_01.jsp");
    //清除
    ...
    $('#tabInfomation4').click(function () {
        bindDATATYPE('B05', 'IFNSH2_OP_FS_REC_CD', ''); //結案紀錄
    ...
    $('#printBtn').click(function () {
        ...
        if (bool) {
            for (var i = 0; i < indexInJqgrid.length; i++) {
                ...
                if (reportType == "OP08A03Q.doc") { //拾得人讓與拾得物所有權收據
                    downloadReport("ReportServlet", {
...
function checkFnshList(inForm) {
    ...
    nowDate = getROCDateSlash(nowDate);
    //結案日期
    if (getADDate($('#IFNSH2_OP_FS_DATE').val().substring(0, 9)) > getADDate(nowDate)) {
        Validator.setMessage("欄位 [ 結案日期 ]：時間需小於系統日期時間！");
        Validator.setBGColor("IFNSH2_OP_FS_DATE");
    }
    Validator.required('IFNSH2_OP_FS_DATE', '結案日期');
    //送交日期
    if (getADDate($('#IPUANDL2_OP_SD_DATE').val().substring(0, 9)) > getADDate(nowDate)) {
        Validator.setMessage("欄位 [ 送交日期 ]：時間需小於系統日期時間！");
        Validator.setBGColor("IPUANDL2_OP_SD_DATE");
    }
...
function saveFnsh() {
    ...
    var OPENLLevel_NM;
    var SD_OPENLLevel_NM;
    //結案單位
    if ($('#OP_FS2_unitLevel4').val() != '') {
        OPENLLevel_NM = $('#OP_FS2_unitLevel2 option:selected').text();
        OPENLLevel_NM += $('#OP_FS2_unitLevel3 option:selected').text();
        OPENLLevel_NM += $('#OP_FS2_unitLevel4 option:selected').text();
    } else if ($('#OP_FS2_unitLevel3').val() != '') {
        OPENLLevel_NM = $('#OP_FS2_unitLevel2 option:selected').text();
        OPENLLevel_NM += $('#OP_FS2_unitLevel3 option:selected').text();
    } else {
        OPENLLevel_NM = $('#OP_FS2_unitLevel2 option:selected').text();
    ...
    //送交單位
    if ($('#OP_SD2_unitLevel4').val() != '') {
        SD_OPENLLevel_NM = $('#OP_SD2_unitLevel2 option:selected').text();
        SD_OPENLLevel_NM += $('#OP_SD2_unitLevel3 option:selected').text();
        SD_OPENLLevel_NM += $('#OP_SD2_unitLevel4 option:selected').text();
    } else if ($('#OP_SD2_unitLevel3').val() != '') {
        SD_OPENLLevel_NM = $('#OP_SD2_unitLevel2 option:selected').text();
        SD_OPENLLevel_NM += $('#OP_SD2_unitLevel3 option:selected').text();
    } else {
        SD_OPENLLevel_NM = $('#OP_SD2_unitLevel2 option:selected').text();
    ...
    if (bool) {
        ...
        var ajData = {
            ...
            'IFNSH_OP_SEQ_NO': $('#IFNSH_OP_SEQ_NO').val(),
            'IPUANDL_OP_SEQ_NO': $('#IPUANDL_OP_SEQ_NO').val(),
            'OP_BASIC_SEQ_NO': $('#IPUBASIC_OP_SEQ_NO').val(),
            'OP_AC_RCNO': $('#IPUBASIC_OP_AC_RCNO').val(),
            'OP_FS_UNITLEVEL2': $('#OP_FS2_unitLevel2').val(),
            'OP_FS_UNITLEVEL2_NM': $('#OP_FS2_unitLevel2').val() == '' ? '' : $('#OP_FS2_unitLevel2 option:selected').text(),
            'OP_FS_UNITLEVEL3': $('#OP_FS2_unitLevel3').val(),
            'OP_FS_UNITLEVEL3_NM': $('#OP_FS2_unitLevel3').val() == '' ? '' : $('#OP_FS2_unitLevel3 option:selected').text(),
            'OP_FS_UNITLEVEL4': $('#OP_FS2_unitLevel4').val(),
            'OP_FS_UNITLEVEL4_NM': $('#OP_FS2_unitLevel4').val() == '' ? '' : $('#OP_FS2_unitLevel4 option:selected').text(),
            'OP_FS_UNITLEVEL_NM': OPENLLevel_NM,
            'OP_FS_STAFF_NM': $('#IFNSH2_OP_FS_STAFF_NM').val(),
            'OP_FS_DATE': $('#IFNSH2_OP_FS_DATE').val(),
            'OP_FS_REC_CD': $('#IFNSH2_OP_FS_REC_CD').val(),
            'OP_FS_REC_NM': $('#IFNSH2_OP_FS_REC_CD').val() == '' ? '' : $('#IFNSH2_OP_FS_REC_CD option:selected').text(),
            'OP_FS_STAT_DESC': $('#IFNSH2_OP_FS_STAT_DESC').val(),
            'OP_NTC_MODE_CD': $("input[name='IPUANDL2.OP_NTC_MODE']:checked").val(),
            'OP_NTC_MODE_NM': $("input[name='IPUANDL2.OP_NTC_MODE']:checked").text(),
            'OP_SD_UNITLEVEL2': $('#OP_SD2_unitLevel2').val(),
            'OP_SD_UNITLEVEL2_NM': $('#OP_SD2_unitLevel2').val() == '' ? '' : $('#OP_SD2_unitLevel2 option:selected').text(),
            'OP_SD_UNITLEVEL3': $('#OP_SD2_unitLevel3').val(),
            'OP_SD_UNITLEVEL3_NM': $('#OP_SD2_unitLevel3').val() == '' ? '' : $('#OP_SD2_unitLevel3 option:selected').text(),
            'OP_SD_UNITLEVEL4': $('#OP_SD2_unitLevel4').val(),
            'OP_SD_UNITLEVEL4_NM': $('#OP_SD2_unitLevel4').val() == '' ? '' : $('#OP_SD2_unitLevel4 option:selected').text(),
            'OP_SD_UNITLEVEL_NM': SD_OPENLLevel_NM,
            'OP_SD_NAME': $('#IPUANDL2_OP_SD_NAME').val(),
            'OP_SD_TITLE': $('#IPUANDL2_OP_SD_TITLE').val(),
            'OP_SD_DATE': $('#IPUANDL2_OP_SD_DATE').val(),
            'OP_AC_REG_ATNO': $('#IPUANDL2_OP_AC_REG_ATNO').val(),
            'OP_AC_NAME': $('#IPUANDL2_OP_AC_NAME').val(),
            'OP_AC_TITLE': $('#IPUANDL2_OP_AC_TITLE').val(),
            'ACTION_FROM': 'IFNSH'
        };
        var ajSucc = function (JData) {
            if (JData.formData) {
