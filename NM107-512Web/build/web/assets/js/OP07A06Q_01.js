...
function registButton() {
    ...
    $('#printBtnPDF').click(function () {
            ...
            //通知拾得人領回日期起迄
            if( ($('#OP_NTC_PUPO_DT_S').val() != "") && ($('#OP_NTC_PUPO_DT_E').val() != "") ){
                if (($('#OP_NTC_PUPO_DT_S').val() > $('#OP_NTC_PUPO_DT_E').val())){
                    msg += '通知拾得人領回日期迄日不可小於通知拾得人領回日期起日<br/>';
                    bool = false;
                }
            }
            if( getADDate($('#OP_NTC_PUPO_DT_S').val().substring(0,9)) >getADDate ( nowDate) ){
                msg += '通知拾得人領回日期起日不可大於系統日期時間<br/>';
                bool = false;
            }
            if( getADDate($('#OP_NTC_PUPO_DT_E').val().substring(0,9)) >getADDate ( nowDate) ){
            ...
            if (bool){
                downloadReport("ReportServlet", {
                    ajaxAction: 'OP07A06Q.doc',
                    reportName: 'oP07A06Q.doc',
                    newReportName: '拾得人領回公告單.doc',
                    OP_AC_D_UNIT_CD: $('#OP_Search_unitLevel2').val(),
                    OP_AC_B_UNIT_CD: $('#OP_Search_unitLevel3').val(),
                    OP_AC_UNIT_CD: $('#OP_Search_unitLevel4').val(),
                    OP_NTC_PUPO_DT_S:$('#OP_NTC_PUPO_DT_S').val(),
                    OP_NTC_PUPO_DT_E:$('#OP_NTC_PUPO_DT_E').val(),
