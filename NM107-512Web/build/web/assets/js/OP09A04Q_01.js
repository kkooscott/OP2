...
function save(type) {
    ...
    if ($('#OP_DOC_WD1').val().toString() == "" && $('#OP_YN_GET_NO1').val() == "1"){
         msg += "招領公告發文字為空白時，自動取號不可為『是』！<br/>";
        bool =false;
    }
    if ($('#OP_DOC_WD2').val().toString() == "" && $('#OP_YN_GET_NO2').val() == "1"){
         msg += "領回公告發文字為空白時，自動取號不可為『是』！<br/>";
         bool =false;
    }
    if( type == 'INS' ){ //新增
        if ($('#OP_unitLevel2_INS').val() != "" && $('#OP_unitLevel3_INS').val() != ""){
            if( checkUnitForDocWd( $('#OP_unitLevel2_INS').val(), $('#OP_unitLevel3_INS').val() ) == "Y" ){
    ...
    if (bool) {
       show_BlockUI_page_noParent('資料儲存中…');
        var ajData = {
            'ajaxAction': type == 'INS' ? 'doSaveByOP09A04Q' : 'doUpdateByOP09A04Q',
            ...
            'OP_DOC_WD1': $('#OP_DOC_WD1').val(),
            'OP_YN_GET_NO1': $('#OP_YN_GET_NO1').val(),
            'OP_DOC_WD2': $('#OP_DOC_WD2').val(),
...
function showGridListbyQS() {
	...
	if (bool){
		...
		$(gridId).jqGrid({
			...
			height : "auto",
			autowidth : true,
			colNames : [ "單位", "招領公告發文字","是否開啟自動取號", "領回公告發文字","是否開啟自動取號", "OP_SEQ_NO","OP_B_UNIT_CD","OP_D_UNIT_CD","OP_YN_GET_NO1","OP_YN_GET_NO2" ],
			colModel : [ {
				name : 'OP_B_UNIT_NM',
				index : 'OP_B_UNIT_NM',
				width : 70
				...
				name : 'OP_YN_GET_NO1_CHT',
				index : 'OP_YN_GET_NO1_CHT',
				width : 40
                        },  {
                                name : 'OP_DOC_WD2',
				index : 'OP_DOC_WD2',
				...
				name : 'OP_YN_GET_NO2_CHT',
				index : 'OP_YN_GET_NO2_CHT',
				width : 40
                       }, {
				name : 'OP_SEQ_NO',
				index : 'OP_SEQ_NO',
			...
			onSelectRow : function(id, status) {
				...
				$('#updateCheckBtn').show();
                            ...
                            $('#OP_DOC_WD1').val(row.OP_DOC_WD1);
                            $('#OP_DOC_WD2').val(row.OP_DOC_WD2);
                            $('#OP_YN_GET_NO1').val(row.OP_YN_GET_NO1);
                            $('#OP_YN_GET_NO2').val(row.OP_YN_GET_NO2);
			}
		});
...
//CHECK---START
function checkUnitForDocWd( OP_D_UNIT_CD, OP_B_UNIT_CD ) {
    var bool;
    var ajData = {
            'ajaxAction' : 'checkUnitForDocWd',
