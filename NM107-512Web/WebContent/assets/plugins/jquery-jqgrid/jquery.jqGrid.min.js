...
function(){
var b=a(this).parent("tr")[0];o=b.nextSibling;
if(a(this).hasClass("sgcollapsed")){
//    alert("has sgcollapsed Class = true");
    
    j=d.p.id;i=b.id;
    if(d.p.subGridOptions.reloadOnExpand===true||d.p.subGridOptions.reloadOnExpand===false&&!a(o).hasClass("ui-subgrid")){
        k=c>=1?"<td colspan='"+c+"'>&#160;</td>":"";
        n=a(d).triggerHandler("jqGridSubGridBeforeExpand",[j+"_"+i,i]);
        (n=n===false||n==="stop"?false:true)&&a.isFunction(d.p.subGridBeforeExpand)&&(n=d.p.subGridBeforeExpand.call(d,j+"_"+i,i));
        if(n===false)return false;
        a(b).after("<tr role='row' class='ui-subgrid'>"+k+"<td class='ui-widget-content subgrid-cell'><span class='ui-icon "+d.p.subGridOptions.openicon+"'></span></td><td colspan='"+parseInt(d.p.colNames.length-1-m,10)+"' class='ui-widget-content subgrid-data'><div id="+j+"_"+i+" class='tablediv'></div></td></tr>");
        a(d).triggerHandler("jqGridSubGridRowExpanded",[j+"_"+i,i]);
        a.isFunction(d.p.subGridRowExpanded)?d.p.subGridRowExpanded.call(d,j+"_"+i,i):g(b)
    } else 
        a(o).show();
    
    //    alert("has sgcollapsed Class?"+a(this).hasClass("sgcollapsed"));
    a(this).html("<a href='javascript:void(0);'><span class='ui-icon "+d.p.subGridOptions.minusicon+"'></span></a>").removeClass("sgcollapsed").addClass("sgexpanded");
//    alert("has sgcollapsed Class?"+a(this).hasClass("sgcollapsed"));
    d.p.subGridOptions.selectOnExpand&&a(d).jqGrid("setSelection",i);
}
else if(a(this).hasClass("sgexpanded")){
//    alert("has sgcollapsed Class = false");
    n=a(d).triggerHandler("jqGridSubGridRowColapsed",[j+"_"+i,i]);
    if((n=n===false||n==="stop"?false:true)&&a.isFunction(d.p.subGridRowColapsed)){
        i=b.id;n=d.p.subGridRowColapsed.call(d,j+"_"+i,i)
    }
    if(n===false)return false;
    d.p.subGridOptions.reloadOnExpand===true?a(o).remove(".ui-subgrid"):a(o).hasClass("ui-subgrid")&&a(o).hide();
    a(this).html("<a href='javascript:void(0);'><span class='ui-icon "+d.p.subGridOptions.plusicon+"'></span></a>").removeClass("sgexpanded").addClass("sgcollapsed")
}
//alert("has sgcollapsed Class?"+a(this).hasClass("sgcollapsed"));
return false}),p++;!0===d.p.subGridOptions.expandOnLoad&&a(d.rows).filter(".jqgrow").each(function(b,d){a(d.cells[0]).click()});d.subGridXml=function(a,d){b(a,d)};d.subGridJson=function(a,b){h(a,b)}}})},expandSubGridRow:function(c){return this.each(function(){if((this.grid||c)&&!0===this.p.subGrid){var e=a(this).jqGrid("getInd",c,!0);e&&(e=a("td.sgcollapsed",e)[0])&&a(e).trigger("click")}})},collapseSubGridRow:function(c){return this.each(function(){if((this.grid||
