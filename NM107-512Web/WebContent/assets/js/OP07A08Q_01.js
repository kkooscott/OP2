...
function registButton() {
        $('#printBtn').click(function() {
            ...
            //拾得日期起迄
            if( ($('#OP_PU_DATE_S').val() != "") && ($('#OP_PU_DATE_E').val() != "") ){
                if (($('#OP_PU_DATE_S').val() > $('#OP_PU_DATE_E').val())){
                    msg += '拾得日期迄日不可小於拾得日期起日<br/>';
                    bool = false;
                }
            }
            if( getADDate($('#OP_PU_DATE_S').val().substring(0,9)) >getADDate ( nowDate) ){
                msg += '拾得日期起日不可大於系統日期時間<br/>';
                bool = false;
            }
            if( getADDate($('#OP_PU_DATE_E').val().substring(0,9)) >getADDate ( nowDate) ){
            ...
            //結案日期起迄
            if( ($('#OP_FS_DATE_S').val() != "") && ($('#OP_FS_DATE_E').val() != "") ){
                if (($('#OP_FS_DATE_S').val() > $('#OP_FS_DATE_E').val())){
                    msg += '結案日期迄日不可小於結案日期起日<br/>';
                    bool = false;
                }
            }
            if( getADDate($('#OP_FS_DATE_S').val().substring(0,9)) >getADDate ( nowDate) ){
                msg += '結案日期起日不可大於系統日期時間<br/>';
                bool = false;
            }
            if( getADDate($('#OP_FS_DATE_E').val().substring(0,9)) >getADDate ( nowDate) ){
                msg += '結案日期迄日不可大於系統日期時間<br/>';
                bool = false;
            }
        //受理拾得日期、拾得日期及結案日期至少輸入一個條件，且日期區間不得超過一年
        if($('#OP_FS_DATE_S').val() == "" &&  $('#OP_FS_DATE_E').val() == "" && $('#OP_PU_DATE_S').val() == "" &&  $('#OP_PU_DATE_E').val() == "" && $('#OP_AC_DATE_S').val() == "" && $('#OP_AC_DATE_E').val() == "" ){
                msg += '拾得日期、受理日期或結案日期至少輸入一個條件<br/>';
                bool = false;
        }else{
            if( $('#OP_AC_DATE_S').val() == "" && $('#OP_AC_DATE_E').val() == "")
            {
                if( ($('#OP_PU_DATE_S').val() == "") && ($('#OP_PU_DATE_E').val() != "") ){
                    msg += '拾得日期起日不可為空<br/>';
                    bool = false;
                }else if( ($('#OP_PU_DATE_S').val() != "") && ($('#OP_PU_DATE_E').val() == "") ){
                    msg += '拾得日期迄日不可為空<br/>';
                    bool = false;
                }else{
                    if( ($('#OP_PU_DATE_S').val() < $('#OP_PU_DATE_E').val()) && compareOneYear( 'OP_PU_DATE_S', 'OP_PU_DATE_E' ) == false ){
                ...
                if( ($('#OP_FS_DATE_S').val() == "") && ($('#OP_FS_DATE_E').val() != "") ){
                    msg += '結案日期起日不可為空<br/>';
                    bool = false;
                }else if( ($('#OP_FS_DATE_S').val() != "") && ($('#OP_FS_DATE_E').val() == "") ){
                    msg += '結案日期迄日不可為空<br/>';
                    bool = false;
                }else{
                    if( ($('#OP_FS_DATE_S').val() < $('#OP_FS_DATE_E').val()) && compareOneYear( 'OP_FS_DATE_S', 'OP_FS_DATE_E' ) == false ){
            ...
            if (bool){
                downloadReport("ReportServlet", {
                    ajaxAction: 'OP07A08Q.xls',
                    reportName: 'OP07A08Q.xlsx',
                    ...
                    OP_PU_DATE_S:$('#OP_PU_DATE_S').val(),
                    OP_PU_DATE_E:$('#OP_PU_DATE_E').val(),
                    OP_AC_DATE_S:$('#OP_AC_DATE_S').val(),
                    OP_AC_DATE_E:$('#OP_AC_DATE_E').val(),
                    OP_FS_DATE_S:$('#OP_FS_DATE_S').val(),
                    OP_FS_DATE_E:$('#OP_FS_DATE_E').val(),
