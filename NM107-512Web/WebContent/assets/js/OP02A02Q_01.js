...
function registButton() {
    ...
    BasicAndDetailButton("OP02A02Q_01.jsp");
    ...
    $('#tabInfomation2').click(function () {
        ...
        if (checkStatusForBasic != 'N') {
            ...
            if ($('#checkStatusForBasic').val() == "1") { //處理中
                if ($('#IPUBASIC_OP_PU_YN_OV500').val() == "0") { //伍佰元以下
...
//        if ($('#jqg_gridList_3_5').prop('checked'))
//        {
//            downloadReport("ReportServlet", {
    ...
    if (bool) {
        ...
        $(gridId).jqGrid({
            ...
            postData: {
                ...
                OP_DEL_FLAG: '0',
                ACTION_TYPE: 'OP02A02Q_01',
                OPR_KIND: appUseList.OPR_KIND,   //委託查詢 202403
                OPR_PURP: appUseList.OPR_PURP    //委託查詢 202403
            },
            height: "auto",
            autowidth: true,
            colNames: ["收據編號", "受理單位", "拾得人類別", "物品屬性", "拾得人姓名", "身分證/其他證號", "拾得日期時間", "拾得地點", "目前狀態", "BASIC_SEQ"],
...
function updateBasicYnOv500() {
    $.alert.open({
        type: 'confirm',
        content: '此案件為一般受理案件，是否要轉成伍佰元專區案件受理？',
        callback:
                function (button, value) {
                    if (button == 'yes') {
                        show_BlockUI_page_noParent('資料變更中…');
                        $('#IPUBASIC_OP_PU_YN_OV500').val('0');
                        var ajData = {
