...
function registButton() {
        $('#printBtnPDF').click(function () {
            ...
            //公告日期起迄
            if( ($('#OP_AN_DATE_BEG').val() != "") && ($('#OP_AN_DATE_END').val() != "") ){
                if (($('#OP_AN_DATE_BEG').val() > $('#OP_AN_DATE_END').val())){
                    msg += '公告日期迄日不可小於公告日期起日<br/>';
                    bool = false;
                }
            }
            if( getADDate($('#OP_AN_DATE_BEG').val().substring(0,9)) >getADDate ( nowDate) ){
                msg += '公告日期起日不可大於系統日期時間<br/>';
                bool = false;
            }
            if( getADDate($('#OP_AN_DATE_END').val().substring(0,9)) >getADDate ( nowDate) ){
                msg += '公告日期迄日不可大於系統日期時間<br/>';
                bool = false;
            }
            //受理日期起迄
            ...
            if (bool){
                downloadReport("ReportServlet", {
                    ajaxAction: 'OP07A03Q.doc',
                    reportName: 'oP07A03Q.doc',
                    newReportName: '拾得物公告招領公告單列印.doc',
                    OP_AC_D_UNIT_CD: $('#OP_Search_unitLevel2').val(),
                    OP_AC_B_UNIT_CD: $('#OP_Search_unitLevel3').val(),
                    OP_AC_UNIT_CD: $('#OP_Search_unitLevel4').val(),
                    OP_AN_DATE_BEG:$('#OP_AN_DATE_BEG').val(),
                    OP_AN_DATE_END:$('#OP_AN_DATE_END').val(),
