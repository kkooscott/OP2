...
//        }
        ...
        //結案日期起迄
        if( ($('#OP_SEARCH_FS_DATE_S').val() != "") && ($('#OP_SEARCH_FS_DATE_E').val() != "") ){
            if (($('#OP_SEARCH_FS_DATE_S').val() > $('#OP_SEARCH_FS_DATE_E').val())){
                msg += '結案日期迄日不可小於結案日期起日<br/>';
                bool = false;
            }
        }
        if( getADDate($('#OP_SEARCH_FS_DATE_S').val().substring(0,9)) >getADDate ( nowDate) ){
            msg += '結案日期起日不可大於系統日期時間<br/>';
            bool = false;
            }
        if( getADDate($('#OP_SEARCH_FS_DATE_E').val().substring(0,9)) >getADDate ( nowDate) ){
            msg += '結案日期迄日不可大於系統日期時間<br/>';
            bool = false;
            }
            //公告期滿日期、受理日期及結案日期至少輸入一個條件，且日期區間不得超過一年
        if($('#OP_CASE_DATE_S').val() == "" &&  $('#OP_CASE_DATE_E').val() == "" && $('#OP_SEARCH_FS_DATE_S').val() == "" &&  $('#OP_SEARCH_FS_DATE_E').val() == "" && $('#OP_SEARCH_AC_DATE_S').val() == "" && $('#OP_SEARCH_AC_DATE_E').val() == "" ){
                msg += '公告期滿日期、受理日期或結案日期至少輸入一個條件<br/>';
                bool = false;
        }else{
            if( $('#OP_SEARCH_AC_DATE_S').val() == "" && ($('#OP_SEARCH_AC_DATE_E').val() == "")  )
            {
                if( ($('#OP_SEARCH_FS_DATE_S').val() == "") && ($('#OP_SEARCH_FS_DATE_E').val() != "") ){
                    msg += '結案日期起日不可為空<br/>';
                    bool = false;
                }else if( ($('#OP_SEARCH_FS_DATE_S').val() != "") && ($('#OP_SEARCH_FS_DATE_E').val() == "") ){
                    msg += '結案日期迄日不可為空<br/>';
                    bool = false;
                }else{
                    if( ($('#OP_SEARCH_FS_DATE_S').val() < $('#OP_SEARCH_FS_DATE_E').val()) && compareOneYear( 'OP_SEARCH_FS_DATE_S', 'OP_SEARCH_FS_DATE_E' ) == false ){
                        msg += '結案日期起日與結案日期迄日不能超過一年<br/>';
                        bool = false;
                    }
                }
                if( ($('#OP_CASE_DATE_S').val() == "") && ($('#OP_CASE_DATE_E').val() != "") ){
        ...
        if (bool){
            downloadReport("ReportServlet", {
                ajaxAction: 'OP07A11Q.xls',
                reportName: 'oP07A11Q.xlsx',
                newReportName: '遺失人領回清冊.xlsx',
                ...
                OP_FS_DATE_S:$('#OP_SEARCH_FS_DATE_S').val(),
                OP_FS_DATE_E:$('#OP_SEARCH_FS_DATE_E').val(),
                includeYN:$('input[name=includeYN]:checked').val(),
                printPUCP_NM_YN:$('input[name=printPUCP_NM_YN]:checked').val(),
                printPUCP_IDN_YN:$('input[name=printPUCP_IDN_YN]:checked').val(),
                printPUCP_ADDR_YN:$('input[name=printPUCP_ADDR_YN]:checked').val(),
                printPUCP_TEL_YN:$('input[name=printPUCP_TEL_YN]:checked').val(),
                lockType : 'A'
            });
        }
    });
