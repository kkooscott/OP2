...
function registButton() {
    ...
    //批次刪除
    $('#batchPuDeleteBtn').click(function () {
        confirmBatchDelete();
    });
    //分頁1 拾得物受理基本資料
    ...
    if (bool) {
        ...
        $(gridId).jqGrid({
            ...
            onSelectRow: function (id, status) {
                ...
                if (selectedColumnName != 'cb') {
                    $('#actionType').val('deleteBasicList');
                    ...
                    //因為只要求500以下，所以這一塊先mark掉
                  //if (checkStatusForBasic(BasicNo) == "1" && strPermissionData["RolePermission"] != '1') {
                    if (checkStatusForBasic(BasicNo) == "1") {
                        $('#puDeleteLabel').show();
                        $('#puDeleteReason').show();
                        $('#puDeleteBtn').show();
                    } else {
...
function confirmBatchDelete() {
    var bool = true;
    //拾得日期
    if ($('#batchdPuDeleteReason').val() == "") {
        $.alert.open('error', '請填入刪除原因');
        bool = false;
    }
    if (bool) {
        $.alert.open({
...
function BatchDeleteBtn() {
    ...
    for (var i = 0; i < indexInJqgrid.length; i++) {
        ...
        if (selTableCurstat != "處理中") {
            $.alert.open('error', '只能刪除處理中的資料');
            bool = false;
        }
    }
    if (bool) {
        ...
        $.ajax({
            ...
            data: {
                ...
                OP_SEQ_NO: delIDs,
                OP_AC_RCNO: $('#IPUBASIC_OP_AC_RCNO').val(),
                OP_DEL_RSN: $('#batchdPuDeleteReason').val()
            },
            clearForm: true,
            resetForm: true,
            success: function (data, textStatus, xhr, $form) {
                notyMsg('已刪除' + data.formData + ' 筆資料');
                $('#batchdPuDeleteReason').val('');
                //刷新查詢結果
                showGridListbyQS();
            }
        });
