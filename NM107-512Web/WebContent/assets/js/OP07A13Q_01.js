...
function registButton() {
    ...
    $('#printBtn').click(function () {
        ...
        if($('#OP_SEARCH_RT_DATE_S').val() == "" &&  $('#OP_SEARCH_RT_DATE_E').val() == "" && $('#OP_SEARCH_PU_DATE_S').val() == "" &&  $('#OP_SEARCH_PU_DATE_E').val() == "" && $('#OP_SEARCH_AC_DATE_S').val() == "" && $('#OP_SEARCH_AC_DATE_E').val() == "" ){
                msg += '拾得日期、受理日期或發還日期至少輸入一個條件<br/>';
                bool = false;
        }else{
            if( $('#OP_SEARCH_AC_DATE_S').val() == "" && ($('#OP_SEARCH_AC_DATE_E').val() == "")  )
            {
        ...
        if (bool){
            downloadReport("ReportServlet", {
                ajaxAction: 'OP07A13Q.xls',
                reportName: 'oP07A13Q.xlsx',
                newReportName: '受理拾得物發還清冊影印.xlsx',
