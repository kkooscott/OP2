...
//        PuanDlCase();
    ...
    $('#menu01span').addClass("selected");
    $('#menu0101').addClass("active");
    init();
});
    ...
    $('#printBtn').click(function () {
        ...
        if (bool) {
            for (var i = 0; i < indexInJqgrid.length; i++) {
                ...
                } else if (reportType == "oP02A06Q.doc") { //認領遺失物領據
                    downloadReport("ReportServlet", {
...
function showGridListbyQS() {
    ...
    var msg = "";
    //認領填單日期
    if (($('#OP_SEARCH_FM_DATE_S').val() != "") && ($('#OP_SEARCH_FM_DATE_E').val() != "")) {
        if (($('#OP_SEARCH_FM_DATE_S').val() > $('#OP_SEARCH_FM_DATE_E').val())) {
            msg += '認領填單日期迄日不可小於認領填單日期起日<br/>';
            bool = false;
        }
    }
    if (getADDate($('#OP_SEARCH_FM_DATE_S').val().substring(0, 9)) > getADDate(nowDate)) {
        msg += '認領填單日期起日不可大於系統日期時間<br/>';
        bool = false;
    }
    if (getADDate($('#OP_SEARCH_FM_DATE_E').val().substring(0, 9)) > getADDate(nowDate)) {
        msg += '認領填單日期迄日不可大於系統日期時間<br/>';
        bool = false;
    }
    if (msg != "") {
        $.alert.open({
    ...
    if (bool) {
        ...
        $(gridId).jqGrid({
            ...
            postData: {
                ...
                OP_FM_DATE_S: getADDate($("#OP_SEARCH_FM_DATE_S").val()),
                OP_FM_DATE_E: getADDate($("#OP_SEARCH_FM_DATE_E").val()),
                OP_YN_LOSER: $("#OP_SEARCH_YN_LOSER").val(),
                OPR_KIND: appUseList.OPR_KIND,  //202403 查詢用途代號
                OPR_PURP: appUseList.OPR_PURP   //202403 查詢目的
            },
            height: "auto",
            autowidth: true,
            colNames: ["填單日期", "收據編號", "認領人姓名", "認領人證號", "遺失日期時間", "受理日期", "遺失地點", "物品描述", "是否審核", "SEQ", "BASIC_SEQ"],
            colModel: [{
                    name: 'OP_FM_DATE',
                    index: 'OP_FM_DATE',
                    width: 40
                    ...
                    name: 'OP_LOST_OJ_DESC',
                    index: 'OP_LOST_OJ_DESC',
                    width: 80
                }, {
                    name: 'OP_YN_LOSER',
                    index: 'OP_YN_LOSER',
...
function showGridListbyQSC() {
    ...
    if (bool) {
        ...
        $(gridId).jqGrid({
            ...
            datatype: "json",
            postData: {
                ajaxAction: 'PuNetQueryForDownSearch',
                OP_UNITLEVEL2: $('#OP_Search_unitLevel2').val(),
                OP_UNITLEVEL2_NM: $('#OP_Search_unitLevel2').val() == '' ? '' : $('#OP_Search_unitLevel2 option:selected').text(),
                OP_UNITLEVEL3: $('#OP_Search_unitLevel3').val(),
                OP_UNITLEVEL3_NM: $('#OP_Search_unitLevel3').val() == '' ? '' : $('#OP_Search_unitLevel3 option:selected').text(),
                ...
                OP_FM_DATE_E: getADDate($("#OP_SEARCH_FM_DATE_E").val()),
                OP_YN_LOSER: $("#OP_SEARCH_YN_LOSER").val(),
                OPR_KIND: appUseList.OPR_KIND,  //202403
                OPR_PURP: appUseList.OPR_PURP   //202403
            },
            height: "auto",
            autowidth: true,
            colNames: ["填單日期", "收據編號", "認領人姓名", "認領人證號", "遺失日期時間", "受理日期", "遺失地點", "物品描述", "是否審核", "SEQ", "BASIC_SEQ"],
...
function clearDetailData() {
    ...
    $("#MACLabel").hide();
    $("#GET_E_MAC_DATA").hide();
    $('#IPUDETAIL_OP_PHONE_NUM').val('');
    $("#IPUDETAIL_OP_PHONE_NUM").hide();
    $('#IPUDETAIL_OP_PHONE_NUM_2').val('');
    $("#IPUDETAIL_OP_PHONE_NUM_2").hide();
    $("#PHONELabel").hide();
    $("#PHONELabel2").hide();
