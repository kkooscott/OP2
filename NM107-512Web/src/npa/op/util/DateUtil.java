...
public class DateUtil {
    ...
    /**
     *使用tryParse將傳入參數date轉乘Date物件，再將時間格式化成報表所需要的格式HH時mm分 
     * @param time tryParse所能接受的格式
     * @return HH時mm分 如:15時35分 
     */
     ...
     * @return 民國年格式(yyyMMdd)
     */
    public static String convertBirthdayTime(String date, String befRoc){
            String newTime = "";
            String year = "";
            String day = "";
            if( !date.equals("") && date.length() == 8 ){
                ...
                Integer newYear = 0;
                if( befRoc.equals("0") ){ //民前
                    newYear = 1912 - Integer.valueOf(year);
                }else{ //民國
                    newYear = Integer.valueOf(year) - 1911;
                }
                if ( newYear < 100 )
                    newTime = String.format("%03d", newYear) + day;
     ...
     * @return YYY.MM.DD
     */
    public static String to7TwDateTime2(String strDate) {
	String tmp = "";
	if (!"".equals(strDate) && strDate.length() == 8)
		tmp = new StringUtil().MakesUpZero(String.valueOf((Integer.parseInt(strDate.substring(0,4))-1911)),3) + "."
			+ strDate.substring(4,6) + "."
    ...
    /**
     * 將來自DB的日期欄位與時間欄位 西元年月日YYYYMMDD 轉成民國年 YYY/MM/DD  時間 HHmm 轉成 HH:mm
     * 
     * @param strDate YYYYMMDD, strTime HHmm
     * @return YYY/MM/DD HH:mm
     */
    public static String to7TwDateTime(String strDate,String strTime) {
	String tmp = "";
	if (!"".equals(strDate) && strDate.length() == 8)
		tmp = new StringUtil().MakesUpZero(String.valueOf((Integer.parseInt(strDate.substring(0,4))-1911)),3) + "/"
			+ strDate.substring(4,6) + "/"
			+ strDate.substring(6,8);
        if (!"".equals(strTime) && strTime.length() == 4)
                tmp += " " + strTime.substring(0,2) + ":"
                     + strTime.substring(2,4);
        //System.out.println("DateUtil_____tmp____"+tmp);
	return tmp;
    }
    
    /**
     * 將來自DB的日期欄位與時間欄位  時間 HHmm 轉成 HH:mm
     * 
     * @param strDate strTime HHmm
     * @return HH:mm
     */
    public static String to4TwTime(String strTime) {
	String tmp = "";
        if (!"".equals(strTime) && strTime.length() == 4)
                tmp = strTime.substring(0,2) + ":"
                     + strTime.substring(2,4);
    ...
    /**
     * 將來自DB的日期欄位與時間欄位  時間 HHmm 轉成 HH時mm分
     * 
     * @param strDate strTime HHmm
     * @return HH時mm分
     */
	...
	String tmp = "";
        if (!"".equals(strTime) && strTime.length() == 4)
                tmp = strTime.substring(0,2) + "時"
                     + strTime.substring(2,4)+ "分";
	return tmp;
    }
     ...
     * @return YYY年MM月DD日
     */
    public static String to7TwDateWithCB(String strDate) {
	String tmp = "";
	if (!"".equals(strDate) && strDate.length() == 8)
     ...
     * @return YYYYMMDD
     */
    public static String toNormalDateTime(String strDate, String befRoc) {
	String tmp = "";
	if (!"".equals(strDate) && strDate.length() == 7){
            if( !befRoc.equals("") && befRoc.equals("0") ){ //民前
                tmp = String.valueOf((1912-Integer.parseInt(strDate.substring(0,3))))
	 ...
	 * 
	 * @param date
	 *            DB的格式為yyyyMMddHHmmss，直接將DB的欄位傳入即可
	 * @return 西元年格式(yyyy/MM/dd HH:mm:ss)
	 */
	public static String get14DateFormat(String date) {
        ...
        /**
        * 依據傳入的西元年字串取得轉換後的民國年；只有1912年(含)以後的西元年可轉換為民國年。
        * 回傳的民國年為3碼靠右左補零。如[2005] ==> [094]
        *
